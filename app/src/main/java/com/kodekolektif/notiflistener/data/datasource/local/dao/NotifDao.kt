...
interface NotifDao {
    ...
    @Query("SELECT * FROM notif WHERE status = :status AND name IS NOT NULL AND nominal IS NOT NULL AND nominal != 0 ORDER BY created_at DESC LIMIT ${Constant.notifSyncLimit}")
    ...
    @Query("UPDATE notif SET status = :status, validated_at = datetime('now') WHERE uuid = :uuid")
